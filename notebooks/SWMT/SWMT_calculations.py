...
# as this configuration has demonstrated considerable success
...
if __name__ == '__main__':
    ...
    def calculate_SWMT_monthly_mean(expt, session, start_time, end_time,
        ...
        SST = cc.querying.getvar(
            ...
            chunks={'time': '200MB'}) - 273.15
        SST = SST.isel(st_ocean=0)
        # conservative temperature in K
        SSS_PSU = cc.querying.getvar(
            expt, 'surface_salt', session, frequency=frequency,
            chunks={'time': '200MB'})
        ...
        isopycnal_bins = np.arange(27.2, 28.11, 0.01)
        ...
        ds.attrs = {'units': 'm/s'}
        ds.to_netcdf(path_output + filename)
    ...
    filename_SWMT = ('SWMT_for_sig0_27-2_28-1_' + expt + '_' +
    ...
    """Surface water mass transformation (spatial sum) in DSW regions and 
    on Antarctic shelf"""
    ...
    # on Antarctic shelf
    swmt_heat_shelf = (swmt_heat_shelf * area_t/1e6).sum(
        ['xt_ocean','yt_ocean']).compute()
    swmt_salt_shelf = (swmt_salt_shelf * area_t/1e6).sum(
        ['xt_ocean','yt_ocean']).compute()

    swmt_heat_shelf.name = 'binned_heat_transformation'
    ds = swmt_heat_shelf.to_dataset()
    ds['binned_salt_transformation'] = swmt_salt_shelf
    ds.attrs = {'units': 'Sv'}
    ds.to_netcdf(path_output + 'SWMT_on_Antarctic_shelf_for_sig0_27-2_28-1_' +
                 expt + '_' + frequency[0:3:2]  + '_' + str(year) + '.nc')
